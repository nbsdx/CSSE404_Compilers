...
bool Parser::match( BasicToken *t1, BasicToken *t2 )
{
...
RTree *Parser::parse (vector<BasicToken*> pgm) {
        ...
        else if ( dynamic_cast<Separator*>( symbols.top() ) )
        {
            ...
            if (tree.size() > 1) {
                ...
                if (!sub->isLeaf() && sub->degree() > 0) {
                    ...
                    pair<int,int> posi = sub->getPos();
                    tree.top()->setPos(posi.first, posi.second);
                }
            }
        }
        else 
recover_label:    
        if( match( symbols.top(), pgm[0] ) )
        {
            ...
            pair<int,int> posi = Parser::getPos(pgm[0]);
            leaf->setPos(posi.first, posi.second);
...
break_early:                    
    ...
    {
        if( errored )
            cerr << set_color( Red ) << "Parsing: Errors occured." << set_color() << " Check output for details." << set_color() << endl;
        else
            cerr << set_color( Cyan ) << "Parsing: No Errors" << set_color() << endl;
    }
    else
...
pair<int,int> Parser::getPos(BasicToken* t) {
    ReservedWord* rw;
    Operator* op;
    Delimiter* d;
    Identifier* id;
    ...
    if(( rw = dynamic_cast<ReservedWord*>( t ) ))
        return {rw->line(), rw->pos()};
    else if(( op = dynamic_cast<Operator*>( t ) ))
        return {op->line(), op->pos()};
    else if(( d = dynamic_cast<Delimiter*>( t ) ))
        return {d->line(), d->pos()};
    else if(( id = dynamic_cast<Identifier*>( t ) ))
        return {id->line(), id->pos()};
    else if(( num = dynamic_cast<Number*>( t ) ))
        return {num->line(), num->pos()};
    else return {-1,-1};
}
